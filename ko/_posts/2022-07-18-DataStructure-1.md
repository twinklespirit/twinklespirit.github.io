...
* 성능에 영향을 주는 요인
    1. HW/SW 환경
        ...
        * 동일한 algorithm이라도 두 컴퓨터에서의 알고리즘 수행 시간(= 알고리즘 성능)은 다를 것이다. 
...
* 성능에 영향을 주는 요인을 차단하는 방법
    * 가상 컴퓨터 + 가상 언어 + 가상 코드를 사용
        * HW/SW로부터 독립적인 상태
    ...
    1. virtual machine(컴퓨터)
        * 발달 과정
            * Turing Machine -> von Neumonn: RAM(Random Acess Machine)
                * Random Acess Machine(컴퓨터) != Random Acess Memory(메모리)
                * 그러나, 개념적으론 같음
        ...
        * RAM
        ...
        >   * 산술연산 +, -, *, /
        >       * 주의! (%, 반올림, 올림, 버림): RAM에서는 단위연산(X), 강의에서는 단위연산(O)
        >   * 비교연산 >, >=, <, <=, ==, !=
        >       * A < B
    ...
    > $T(n)=2n-1$ 이다. $n=6$ 이라면,$T(6)=12-1=11$ 이다.

    <br>

> 🗣️ 알고리즘 수행시간은 == 최악의 입력에 대한 기본 연산 횟수로 정의한다.
